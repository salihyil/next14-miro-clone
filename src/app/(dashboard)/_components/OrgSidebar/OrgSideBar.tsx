  ...
  return (
    <aside className="hidden lg:flex flex-col space-y-6 w-[206px] pl-5 pt-5">
      ...
      <div className="space-y-1 w-full">
        <Button asChild size={"lg"} variant={favorites ? "ghost" : "secondary"} className="w-full font-normal justify-start px-2">
          <Link href={"/"}>
            <LayoutDashboard className="h-4 w-4 mr-2" />
            Team Board
          </Link>
        </Button>
        <Button asChild size={"lg"} variant={favorites ? "secondary" : "ghost"} className="w-full font-normal justify-start px-2">
          <Link
            href={{
              pathname: "/",
